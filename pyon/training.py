...
class TrainConfig:
    ...
    render_corr: str = "9,3"        # длина корреляции спекла рендерера при обучении "k_h,k_f" (0,0 — iid; сим2реал
                                    # pre 2026-09-05: iid 0.259, corr(5,9) 0.293, corr(7,3)+кратники 0.305 при контроле 0.370)
    render_mult: float = 0.7        # вероятность добавить в ВХОД рендерера кратник 2F2/2Es (h′→2h′; метка без него —
