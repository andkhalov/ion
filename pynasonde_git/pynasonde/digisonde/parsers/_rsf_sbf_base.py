...
class RsfSbfBinaryBlockExtractor:
    ...
    bytes_per_bin: int = 2  # RSF: 2 байта на бин (амплитуда/доплер + фаза/азимут); SBF: 1 байт
        ...
        with open(self.filename, "rb") as file:
            for block_index in range(self.BLOCKS):
                ...
                for _ in range(header.number_of_frequency_groups):
                    ...
                    blk_size -= self.bytes_per_bin * settings["number_range_bins"] + 6
                    if group is None:  # END-OF-IONOGRAM (0xEE) или пустой хвост блока
                        break
    ...
    def _read_frequency_group(self, file, settings: dict):
        ...
        first = rb()
        if first in (0xEE, 0x00):  # EOI-маркер или нулевой хвост блока
            file.read(5 + self.bytes_per_bin * settings["number_range_bins"])
            return None
        pol, group_size = self.unpack_bcd(first, "tuple")
        ...
        n = settings["number_range_bins"]
        raw = np.frombuffer(file.read(self.bytes_per_bin * n), dtype=np.uint8).astype(np.int64)
        if self.bytes_per_bin == 2:
            b1, b2 = raw[0::2], raw[1::2]
            group.amplitude, group.dop_num = b1 >> 3, b1 & 7
            group.phase, group.azimuth = b2 >> 3, b2 & 7
        else:  # SBF: только амплитуда (5 бит) + доплер (3 бита)
            group.amplitude, group.dop_num = raw >> 3, raw & 7
            group.phase = np.zeros(n, dtype=np.int64)
            group.azimuth = np.zeros(n, dtype=np.int64)
        return group
